...
//// Data (let's pretend that we're getting this data from API, and usually data revieved from APIs is object)
...
// func for formating curency, but it could be used in any app, parameters that we pass in func are kinda universal, reusable function that can take any value, locale and currency
...
const startLogOutTimer = function () {
  const tick = function () {
    ...
    // In each call print the remained time
    labelTimer.textContent = `${min}:${sec}`;

    // When 0 seconds, stop timer and log out (hide UI)
    if (time === 0) {
      clearInterval(timer);
